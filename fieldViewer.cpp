...
// THIS FUNCTION
///////////////////////////////////////////////////////////////////////
 bool insideJuliaSet1(float x, float y, float maxRadius, int maxIterations, int& iterations){
  complex<float> q (x, y);
  complex<float> c (-0.74543, 0.11301);
  int i = 0;
  while(sqrt(norm(q)) <= maxRadius && i < maxIterations){
    q = q * q + c;
    i = i + 1;
  }
  if(i == maxIterations){
  ...
  bool insideJuliaSet2(float x, float y, float maxRadius, int maxIterations, int& iterations){
  complex<float> q (x, y);
...
void runOnce()
{
  ...
  for(int y = 0; y < field.yRes(); y++){
    for(int x = 0; x < field.xRes(); x++){
      float real1 = (float)(x)/field.xRes() - 0.5;
      float imagine1 = (float)(y)/field.yRes() - 0.5;
      float real2 = (float)(x * 2.0)/field.xRes() - 0.5;
      float imagine2 = (float)(y * 2.0)/field.yRes() - 0.5;
      int iterations = 0;
      if(insideJuliaSet1(real1, imagine1, 2.0, 100, iterations)
      && insideJuliaSet2(real2, imagine2, 2.0, 50, iterations)){
        field(x,y) = VEC3F(1.0, (float)(x)/field.xRes(), (float)(y)/field.yRes());
      }else if (insideJuliaSet1(real1, imagine1, 2.0, 100, iterations)){
        field(x,y) = VEC3F((float)(x)/field.xRes(), 1.0 - (float)(x)/field.xRes(), (float)(y)/field.yRes());
      }else if (insideJuliaSet2(real2, imagine2, 2.0, 50, iterations)){
        field(x,y) = VEC3F((float)(x)/field.xRes(), (float)(y)/field.yRes(), 1.0 - (float)(x)/field.xRes());
      }
      else{
        // Assuming maxIterations is 100
        float shade = 1.0f - 1.0f/(float)(iterations);
        field(x,y) = VEC3F(shade, (float)(x)/field.xRes(), (float)(y)/field.yRes());
      }
    }
  }
}
